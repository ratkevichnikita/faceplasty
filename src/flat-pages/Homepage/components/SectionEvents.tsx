...
const SectionEvents:FC<ComponentProps> = ({events}) => {
  ...
  const filteredEvents = events && events
    .filter(item => !item.internalName?.toLowerCase().includes('test'))
    .filter((item) => {
  ...
  return (
    <>
      {(filteredEvents && filteredEvents?.length > 0) ?
            ...
            .map((item) => {
              ...
              return (
                <SwiperSlide key={item.id}
                             className="rounded-[2.08vw] mr-[10px] overflow-hidden border border-[#00000014] shadow-cardShadow max-w-[22.63vw] lg:max-w-[28.13vw] sm:max-w-[75.13vw] sm:rounded-[10.26vw]">
                  <Image
                  ...
                  <div className="p-[1.04vw] space-y-[1.04vw] sm:space-y-[5.13vw] sm:p-[3.08vw]">
                    ...
                    <span
                      className="bg-purple text-white rounded-[2.08vw] border border-transparent font-medium text-[0.83vw] text-center px-[1.04vw] py-[0.31vw] sm:rounded-[10.26vw] sm:py-[1.54vw] sm:px-[5.13vw] sm:text-[3.08vw]">
                      {date}
                    </span>
                      <span
                        className="bg-white border border-[rgba(0,0,0,0.2)] text-black rounded-[2.08vw] font-medium text-[0.83vw] text-center px-[1.04vw] py-[0.31vw] sm:rounded-[10.26vw] sm:py-[1.54vw] sm:px-[5.13vw] sm:text-[3.08vw]">
        ...
        : <p className="text-[0.83vw] sm:text-[4.15vw]">There are no events scheduled yet</p>
      }
    </>
  );
};
